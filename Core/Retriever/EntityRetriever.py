    ...
    async def _find_relevant_entities_vdb(self, seed, tree_node=False, top_k=None):
        try:
            ...
            if not retrieved_nodes_data:
                logger.warning("VDB returned no nodes")
                return None
            ...
            for i, node_data_item in enumerate(retrieved_nodes_data):
                ...
                vdb_score = scores[i] if scores and i < len(scores) and scores[i] is not None else "N/A"
                ...
                # Attach clusters from community_node_map if available
                if hasattr(self, 'community') and self.community and hasattr(self.community, 'community_node_map') and self.community.community_node_map:
                    try:
                        entity_cluster_info = await self.community.community_node_map.get_by_id(node_id)
                        if entity_cluster_info:
                            current_node_data["clusters"] = entity_cluster_info
                    except Exception as e:
                        logger.warning(f"Exception fetching clusters for entity '{node_id}': {e}")

                nodes_with_metadata.append(current_node_data)

            # Add the 'rank' key based on node degree
            if nodes_with_metadata:
                entity_names = [node.get("entity_name") for node in nodes_with_metadata if node.get("entity_name")]
                if entity_names:
                    try:
                        node_degrees = await asyncio.gather(
                            *[self.graph.node_degree(name) for name in entity_names]
                        )
                        name_to_degree = dict(zip(entity_names, node_degrees))
                        for node_dict in nodes_with_metadata:
                            node_dict["rank"] = name_to_degree.get(node_dict.get("entity_name"), 0)
                    except Exception as e:
