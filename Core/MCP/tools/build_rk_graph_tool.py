"""
MCP wrapper for BuildRKGraph tool

Provides MCP-compatible interface for building relation-keyword graphs.
"""
...
class BuildRKGraphMCPTool:
    """MCP wrapper for BuildRKGraph tool"""
    
    def __init__(self):
        self.tool_id = "graph.BuildRKGraph"
        self.name = "graph.BuildRKGraph"
        self.description = "Build a relation-keyword graph from a dataset"
            ...
            "input_schema": {
                ...
                "properties": {
                    ...
                    "config_overrides": {
                        ...
                        "properties": {
                            "keywords_num": {"type": "integer"},
                            "keywords_top_k": {"type": "integer"},
    ...
    async def execute(self, params: Dict[str, Any], session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if params.get("config_overrides"):
                config_overrides = RKGraphConfigOverrides(**params["config_overrides"])
            ...
            if progress_callback:
                await progress_callback({
                    "status": "started",
                    "message": f"Building RK graph for {inputs.target_dataset_name}",
                    "progress": 0
                })
            
            # Execute tool
            logger.info(f"MCP: Executing build_rk_graph for {inputs.target_dataset_name}")
            ...
            logger.info(f"MCP: build_rk_graph completed in {execution_time:.1f}ms")
            
            return result_dict
            
        except Exception as e:
            logger.error(f"MCP: build_rk_graph failed: {e}")
            ...
            return {
                "graph_id": f"{params.get('target_dataset_name', 'unknown')}_RKGraph",
                "status": "failure",
                "message": str(e),
                "node_count": None,
                "edge_count": None,
...
build_rk_graph_mcp_tool = BuildRKGraphMCPTool()