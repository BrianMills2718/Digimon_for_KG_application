"""
MCP wrapper for BuildPassageGraph tool

Provides MCP-compatible interface for building passage-based graphs.
"""
...
class BuildPassageGraphMCPTool:
    """MCP wrapper for BuildPassageGraph tool"""
    
    def __init__(self):
        self.tool_id = "graph.BuildPassageGraph"
        self.name = "graph.BuildPassageGraph"
        self.description = "Build a passage-based graph from a dataset"
            ...
            "input_schema": {
                ...
                "properties": {
                    ...
                    "config_overrides": {
                        ...
                        "properties": {
                            "passage_window_size": {"type": "integer"},
                            "passage_overlap": {"type": "integer"},
    ...
    async def execute(self, params: Dict[str, Any], session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if params.get("config_overrides"):
                config_overrides = PassageGraphConfigOverrides(**params["config_overrides"])
            ...
            if progress_callback:
                await progress_callback({
                    "status": "started",
                    "message": f"Building passage graph for {inputs.target_dataset_name}",
                    "progress": 0
                })
            
            # Execute tool
            logger.info(f"MCP: Executing build_passage_graph for {inputs.target_dataset_name}")
            ...
            logger.info(f"MCP: build_passage_graph completed in {execution_time:.1f}ms")
            
            return result_dict
            
        except Exception as e:
            logger.error(f"MCP: build_passage_graph failed: {e}")
            ...
            return {
                "graph_id": f"{params.get('target_dataset_name', 'unknown')}_PassageGraph",
                "status": "failure",
                "message": str(e),
                "node_count": None,
                "edge_count": None,
...
build_passage_graph_mcp_tool = BuildPassageGraphMCPTool()