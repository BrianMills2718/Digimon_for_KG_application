"""
MCP wrapper for BuildTreeGraph tool

Provides MCP-compatible interface for building hierarchical tree graphs.
"""
...
class BuildTreeGraphMCPTool:
    """MCP wrapper for BuildTreeGraph tool"""
    
    def __init__(self):
        self.tool_id = "graph.BuildTreeGraph"
        self.name = "graph.BuildTreeGraph"
        self.description = "Build a hierarchical tree graph from a dataset"
    ...
    async def execute(self, params: Dict[str, Any], session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if params.get("config_overrides"):
                config_overrides = TreeGraphConfigOverrides(**params["config_overrides"])
            ...
            if progress_callback:
                await progress_callback({
                    "status": "started",
                    "message": f"Building tree graph for {inputs.target_dataset_name}",
                    "progress": 0
                })
            
            # Execute tool
            logger.info(f"MCP: Executing build_tree_graph for {inputs.target_dataset_name}")
            
            result = await build_tree_graph(
                tool_input=inputs,
            ...
            logger.info(f"MCP: build_tree_graph completed in {execution_time:.1f}ms")
            
            return result_dict
            
        except Exception as e:
            logger.error(f"MCP: build_tree_graph failed: {e}")
            ...
            return {
                "graph_id": f"{params.get('target_dataset_name', 'unknown')}_TreeGraph",
                "status": "failure",
                "message": str(e),
                "node_count": None,
                "edge_count": None,
...
build_tree_graph_mcp_tool = BuildTreeGraphMCPTool()