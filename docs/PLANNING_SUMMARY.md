...
The wiki/catalog must describe semantic organization and actual representations, schemas, canonical join keys, capabilities and evidence. Native harness search/file/wiki navigation should not be wrapped merely for symmetry. Analytics remain first-class; source evidence, computed state and findings remain distinct; the provenance graph is not the domain graph.
...
These are scoped synthetic execution receipts, not full repository/provider/MCP/real-corpus or autonomous-harness certification. [CURRENT_STATE.md](CURRENT_STATE.md) distinguishes the tested path from the carried-forward wider source inventory.
...
The [ROADMAP](ROADMAP.md) retains additional representation/retrieval families, broader analytics, stronger lineage/invalidation, Python/CLI/MCP convergence, legacy cleanup and later evaluation. The first slice does not replace that larger vision. No geospatial, new UI, generic planner/multi-agent platform, or production scaling is added here.
